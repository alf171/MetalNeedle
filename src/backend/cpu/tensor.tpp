...
void Tensor<T>::initialize(py::bytes bytes_data,
  ...
  for (size_t i = 0; i < total_size; i++) {
    data_vec.push_back(src[i] / normalize);
  }
...
template <typename T>
Tensor<T> Tensor<T>::create(const std::vector<T> &data,
                            const std::vector<size_t> &shape) {
...
template <typename T>
std::vector<size_t>
Tensor<T>::m_calculate_stride(const std::vector<size_t> &input_shape) {
...
template <typename T>
size_t
Tensor<T>::mult_dim_to_flat_index(const std::vector<size_t> &dimension) const {
  if (dimension.size() != this->shape.size()) {
    throw std::invalid_argument("[mult_dim_to_flat_index] expected dim of " +
                                std::to_string(this->shape.size()) +
                                " but got " + std::to_string(dimension.size()));
  }
...
template <typename T>
std::vector<size_t>
Tensor<T>::flat_index_to_mult_dim(const size_t index) const {
...
template <typename T> int Tensor<T>::get_tensor_count() {
...
Tensor<T> Tensor<T>::max(const std::vector<size_t> &axes,
  ...
  std::vector<T> data(this->m_calculate_size(shape),
                      std::numeric_limits<T>::lowest());
  Tensor<T> result_tensor =
      Tensor<T>(data, shape, m_calculate_stride(shape), 0);

  for (size_t i = 0; i < this->total_size; i++) {
    std::vector<size_t> multi_dim = this->flat_index_to_mult_dim(i);
    std::vector<size_t> result_indices;
    // if we reduce across whole tensor, we need to ensure we have at least on
    // value so we index into [1] properly
  ...
  pybind11::class_<Tensor<T>>(m, class_name.c_str())
      ...
      .def("initialize",
           py::overload_cast<const std::vector<T> &,
               ...
               &Tensor<T>::initialize),
           "Initialize a Tensor with vector data", pybind11::arg("data"),
           pybind11::arg("shape"))
      .def("initialize",
           static_cast<void (Tensor<T>::*)(py::bytes,
                                           const std::vector<size_t> &, float)>(
               &Tensor<T>::initialize),
           "Initialize a Tensor with bytes data", pybind11::arg("bytes_data"),
           pybind11::arg("shape"), pybind11::arg("normalize") = 255.0f)
      .def_static(
          "create",
          [](const std::vector<T> &data, const std::vector<size_t> &shape) {
