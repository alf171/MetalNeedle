...
struct Tensor {
  ...
  static Tensor<T> initialize(const std::vector<T>& data, const std::vector<size_t>& shape){
        ...
        id<MTLFunction> computeFunction = [defaultLibrary newFunctionWithName:@"add_arrays"];
        id<MTLComputePipelineState> computePipelineState = [device newComputePipelineStateWithFunction:computeFunction error:&error];
...
PYBIND11_MODULE(cpu_backend, m) {
    bind_operations<int32_t>(m, "GPUBackendInt");
    bind_operations<int64_t>(m, "GPUBackendLong");
    bind_operations<float>(m, "GPUBackendFloat");
    bind_operations<double>(m, "GPUBackendDouble");
